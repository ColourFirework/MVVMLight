...
public class NewsViewModel implements ViewModel {
    ...
    public final ReplyCommand<Integer> onLoadMoreCommand = new ReplyCommand<>((p) -> {
        loadNewsList(news.getDate(), null);
    }, () -> !viewStyle.isRefreshing.get());
    ...
    public void loadNewsList(String date, Subject subject) {
        ...
        newsListOb.doAfterTerminate(()->viewStyle.isRefreshing.set(false));

        if (subject != null) {
            newsListOb.subscribe(subject);
        }
    }
